...
# ============================================================================
# export_header: emit a C header from all local types in the IDB
# ============================================================================


class ExportHeaderResult(TypedDict, total=False):
    output_path: str
    types_exported: int
    bytes_written: int
    ok: bool
...
def export_header(
    output_path: Annotated[
        str,
        "Destination file path for the generated C header (e.g. /tmp/types.h). "
        "Use an empty string or omit to return header text inline instead of writing a file.",
    ] = "",
    filter: Annotated[
        str,
        "Glob/wildcard pattern to restrict which type names are exported (default: * = all). "
        "Examples: 'MY_*', '*_t', 'Foo*'.",
    ] = "*",
    max_preview: Annotated[
        int,
        "Max characters of header text to include in the 'preview' field of the response "
        "(0 = no preview, -1 = full text). Default: 2000.",
    ] = 2000,
) -> ExportHeaderResult:
    """Export all matching local types from the IDB as a C header file.

    Iterates every type ordinal in the local type library (structs, enums, unions,
    typedefs) and emits them as C declarations using IDA's print_tinfo() formatter.
    Optionally writes the result to a file.

    Useful for:
    - Carrying type knowledge across IDA sessions
    - Sharing recovered struct definitions with other tools (e.g. pwndbg, Binary Ninja)
    - Generating a header for use with import_header or ctypes

    Returns the number of exported types, the output path, and an optional inline
    preview of the first max_preview characters.
    """
    import fnmatch as _fnmatch

    til = ida_typeinf.get_idati()
    if til is None:
        return {"ok": False, "error": "Could not access local type library", "types_exported": 0}

    ordinal_qty = ida_typeinf.get_ordinal_qty(til)
    if ordinal_qty <= 0:
        return {
            "ok": True,
    ...
    lines: list[str] = [
        "/* IDA Pro exported type header */",
        "/* Generated by export_header MCP tool */",
        "",
        "#pragma once",
        "#include <stdint.h>",
        "#include <stdbool.h>",
    ...
    exported = 0
    glob_pat = filter.strip() or "*"

    for ordinal in range(1, ordinal_qty + 1):
        tif = ida_typeinf.tinfo_t()
        ...
        name = tif.get_type_name()
        if not name:
            # Anonymous types: try ordinal-based name
            name = f"__anon_{ordinal}"

        # Apply filter
        if glob_pat != "*" and not _fnmatch.fnmatch(name, glob_pat):
            continue
        ...
        try:
            decl = ida_typeinf.print_tinfo(
                ...
                0,    # indent
                0,    # cmtindent
                ida_typeinf.PRTYPE_DEF | ida_typeinf.PRTYPE_SEMI | ida_typeinf.PRTYPE_NOARGS,
                tif,
                name,
                "",   # cmt
            )
            ...
            # Fallback: minimal declaration
            try:
                kind = tif.get_size()
                lines.append(f"/* {name}: could not format (size={kind}) */")
            except Exception:
                lines.append(f"/* {name}: export failed */")
    ...
    preview = ""
    if max_preview != 0:
        if max_preview < 0 or max_preview >= len(header_text):
            preview = header_text
        else:
            preview = header_text[:max_preview] + f"\n... ({len(header_text) - max_preview} more chars)"
